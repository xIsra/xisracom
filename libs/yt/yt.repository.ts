...
const createGetChannelUrl = (username: string) =>
  `https://www.googleapis.com/youtube/v3/channels?part=contentDetails&forUsername=${username}&key=${YOUTUBE_API_KEY}`;

const createGetPlaylistUrl = (playlistId: string) =>
  `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=50&playlistId=${playlistId}&key=${YOUTUBE_API_KEY}`;

async function _getVideosByPlaylist(
  playlistId: string
